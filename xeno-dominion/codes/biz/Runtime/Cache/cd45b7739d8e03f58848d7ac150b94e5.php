...
<script>
	...
	function del_img(img_name,filename){
		if(confirm("Remove?")){
			...
			$.ajax({ 
					type: "post", 
					url: "<?php echo ($SITE_URL); ?>/biz.php?s=/Product/del_img/pid/<?php echo ($_GET['pid']); ?>/img_name/"+img_name, 
					dataType: "json", 
...
<table width="98%" align="center" border="0" cellpadding="3" cellspacing="1" bgcolor="#CBD8AC" style="margin-bottom:8px;margin-top:8px;">
  ...
  <tr bgcolor="#FFFFFF">
	...
	<select name="location_code" id="location_code_id">
	  <?php if(is_array($all_regions)): foreach($all_regions as $key=>$vo): ?><option value="<?php echo ($vo["region_code"]); ?>" <?php  if($vo["region_code"] == $data["tb_products"]["location_code"]){ ?>selected="selected" <?php } ?>><?php echo ($vo["region_country"]); ?>/<?php echo ($vo["region_province"]); ?></option><?php endforeach; endif; ?>
	</select>	</td>
  </tr>
  <tr bgcolor="#FFFFFF">
