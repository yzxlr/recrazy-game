...
function del(id){
	...
	{
	  if(confirm("Delete?")){ 
	    location="__URL__/lang_del/post_id/<?php echo ($_GET['post_id']); ?>/post_lang_id/"+id;
      }
	}
}
</script>
        ...
        <table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px; margin-bottom:0px;">
            <tr bgcolor="#E7E7E7">
                <td height="30" colspan="10">
                    <span style="float:left; padding-top:3px; padding-left:5px;">Post Additional Language List</span>
                    <span style="float:right;"><a href="__URL__/lang_add/post_id/<?php echo ($_GET['post_id']); ?>">Add New Post Language</a></span>
                </td>
            </tr>
            ...
            <?php if(is_array($data["tb_post_lang"])): foreach($data["tb_post_lang"] as $key=>$vo): ?><tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
            	<td><?php echo ($vo["post_lang_id"]); ?></td>
                <td><?php echo ($vo["post_id"]); ?></td>
                <td><?php echo ($vo["post_title"]); ?></td>
                <td><?php echo ($vo["post_name"]); ?> (<?php echo ($vo["lang_code"]); ?>)</td>
                <td>
                    <a href="__URL__/lang_update/post_lang_id/<?php echo ($vo["post_lang_id"]); ?>/post_id/<?php echo ($_GET['post_id']); ?>">Update</a> | 
                    <a href="javascript:del(<?php echo ($vo["post_lang_id"]); ?>)">Delete</a>
                </td>
