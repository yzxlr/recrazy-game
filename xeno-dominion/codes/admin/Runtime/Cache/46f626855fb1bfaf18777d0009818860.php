...
function del(id){
	...
	{
	  if(confirm("Delete?")){ 
	    location="__URL__/del/post_id/"+id;
      }
	}
}
</script>
        ...
        <table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px; margin-bottom:0px;">
            <tr bgcolor="#E7E7E7">
                <td height="30" colspan="10">
                    ...
                    <span style="float:right;"><a href="__URL__/add">Add New Post</a></span>
                </td>
            </tr>
            ...
            <?php if(is_array($data["tb_post"])): foreach($data["tb_post"] as $key=>$vo): ?><tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
                <td><?php echo ($vo["post_id"]); ?></td>
                <td><?php echo ($vo["post_title"]); ?></td>
                <td><?php echo ($vo["post_lang"]); ?></td>
                <td><?php echo ($vo["post_status"]); ?></td>
                <td><?php if(!empty($vo["post_create"]))echo $vo["post_create"]; else echo "--";  ?></td>
                <td><?php if(!empty($vo["post_modify"]))echo $vo["post_modify"]; else echo "--"; ?></td>
                <td>
                    <a href="__URL__/update/post_id/<?php echo ($vo["post_id"]); ?>">Update</a> | 
                    <a href="javascript:del(<?php echo ($vo["post_id"]); ?>)">Delete</a>
                </td>
                <td><a target="_blank" href="<?php echo ($SITE_URL); ?>/admin.php?s=Post/lang_index/post_id/<?php echo ($vo["post_id"]); ?>">List Languages</a></td>
