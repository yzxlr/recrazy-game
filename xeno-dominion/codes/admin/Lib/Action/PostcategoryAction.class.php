<?php
//网站分类管理
class postCategoryAction extends CommonAction{
	...
	public function lang_index(){
		//0. Initialization
		$tb_postCat = M("postCat");
		$tb_postCatLang = M("postCatLang");
		...
		if(!empty($_GET['cat_id'])){
			$data["tb_postCat"]=$tb_postCat -> where(array("cat_id"=>$_GET["cat_id"])) -> find();
		}
		
		//2. Table products_cat_lang
		if(!empty($_GET['cat_id'])){
			$condition = array("cat_id"=>$_GET['cat_id']);
			$data["postCatLang"]=$tb_postCatLang->where($condition)->select();
		}
	...
	public function lang_edit(){
		...
		if(!empty($_POST['cat_id'])){
			...
			$data3["lang_name"]= $the_lang["language"];
			/////var_dump($data3);
			if($tb_postCatLang->where(array("catlang_id"=>$_GET["catlang_id"]))->save($data3)){
		...
		if(!empty($_GET['catlang_id'])){
			$data["tb_postCatLang"]=$tb_postCatLang -> where(array("catlang_id"=>$_GET["catlang_id"])) -> find();
		}
	...
	public function lang_del(){
		$tb_postCatLang = M("postCatLang");
		if(!empty($_GET['catlang_id'])){
			if($tb_postCatLang->where(array("catlang_id"=>$_GET['catlang_id']))->delete()){
